...
def add(new_data: Type_Add_Data, table: str = "default") -> Type_Add_Return:
	...
	if isinstance(new_data, dict):
		...
		new_row = {"id": get_next_id(table), "rev": 1, "table": table, "data": deepcopy(new_data)}
		...
		for row in new_data:
			...
			new_row = {"id": get_next_id(table), "rev": 1, "table": table, "data": deepcopy(row)}
			database.append(new_row)
...
def find_one(id: int = 0, data: dict = {}, table: str = "default", rev: int = 0) -> Type_Find_One_Return:
